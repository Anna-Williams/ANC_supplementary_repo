---
title: "Compositional analysis with Milo - oligodendroglia subclusters"
...
Oligos_sce <- as.SingleCellExperiment(Oligos, assay = "integrated")
...
```{r, dev="jpeg"}
plotReducedDim(Oligos_sce, colour_by="donor_status", dimred = "UMAP", text_by = "ol_clusters_named") 
```
...
Oligos_milo <- buildGraph(Oligos_milo, k = 30, d = 13, reduced.dim = "PCA")
```
...
```{r}
Oligos_milo <- makeNhoods(Oligos_milo, prop = 0.1, k = 10, d=13, refined = TRUE, reduced_dims = "PCA")
...
```{r}
Oligos_milo <- countCells(Oligos_milo, meta.data = as.data.frame(colData(Oligos_milo)), sample="ProcessNumber")
```
...
```{r}
design <- data.frame(colData(Oligos_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
```{r}
Oligos_milo <- calcNhoodDistance(Oligos_milo, d=13, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(Oligos_milo, design = ~ Tissue + donor_status, design.df = design)
head(da_results)
```
...
umap_pl <- plotReducedDim(Oligos_milo, dimred = "UMAP", colour_by="donor_status", text_by = "ol_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(Oligos_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(Oligos_milo, da_results, coldata_col = "ol_clusters_named")
#da_results <- annotateNhoods(Oligos_milo, da_results, coldata_col = "celltype")
head(da_results)
```
...
```
```{r}
da_results$ol_clusters_named <- ifelse(da_results$ol_clusters_named_fraction < 0.7, "Mixed", da_results$ol_clusters_named)
```
...
```{r}
#save results
saveRDS(da_results, here("Oligos_da_results.rds"))
```
