---
title: "Excitatory neuron clustering"
...
ENs <-subset(seur_comb, idents = c("EN1_Granule", "EN2_CUX2", "EN3_SPARCL1", "EN4_Hipp", "EN5", "EN6_Hipp", "EN7", "EN8"))
...
VlnPlot(ENs, features = c("nFeature_RNA", 
                             "nCount_RNA", 
                             "percent.mt"), 
        pt.size = 0, 
        cols = mycoloursP)
...
```{r}
plot1 <- FeatureScatter(ENs, 
                        feature1 = "nCount_RNA", 
                        feature2 = "percent.mt", 
                        cols = mycoloursP)

plot2 <- FeatureScatter(ENs, 
                        feature1 = "nCount_RNA", 
...
DefaultAssay(ENs) <-"integrated"

ENs <- FindVariableFeatures(ENs, 
                               selection.method = "vst", 
                               nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(ENs), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(ENs)
...
```{r}
ENs <- RunPCA(ENs, features = VariableFeatures(object = ENs))
ElbowPlot(ENs)

```

```{r}
#ENs@graphs$integrated_snn <- "integrated_snn"

ENs <- FindNeighbors(ENs, dims = 1:9)

# test different resolutions for clustering
ENs <- FindClusters(ENs, resolution = c(0.04, 0.06, 0.08,
                                              seq(from = 0.1, to = 1, by = 0.1)), graph.name = "integrated_snn")

ENs@meta.data$integrated_snn_res.0.25 <- NULL

# non-linear reduction
ENs <- RunUMAP(ENs, dims = 1:9)
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out")

dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/diff_dim_pl")
plot_list_func(seur_obj = ENs, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/diff_dim_pl/" )
...
DimPlot(ENs, group.by = "Tissue", split.by = "Tissue")
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/clust_tree")

pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/clust_tree/clust_tree.pdf", 
...
```{r}
ENs_sce <- as.SingleCellExperiment(ENs, assay = "integrated")
#ENs_sce <- readRDS("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/20211027_ENs_SCE.rds")
```
...
sil_plot <- function(sce_obj, reduction = "PCA",
  ...
  met_dat <- as.data.frame(colData(ENs_sce))
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/sil_pl")

av_df <- sil_plot(sce_obj = ENs_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/sil_pl/" )
...
pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/sil_pl/av_sil_pl.pdf", 
     width=8, height=4)
print(av_sil_pl)
dev.off()
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/appr_sil")


approx_sil(sce_obj = ENs_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/appr_sil/" )
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/clust_pure")


clu_pure(sce_obj = ENs_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/clust_pure/")
...
```{r}
Idents(ENs) <- "integrated_snn_res.0.2"
  ENs <-  RenameIdents(
    ENs,
    "0" = "Granule_1",
    "1" = "Ex_1_cx",
   ...
   "9" = "Ex_7_cx",
    ...
    "11" = "Ex_9_hc")
# Plot result
DimPlot(ENs,label = TRUE, repel=TRUE)
dim_pl <- DimPlot(ENs,label = TRUE, repel=FALSE, cols = mycoloursP[6:40],
                  label.size = 4.5)
dim_pl
# Save result
ENs$en_clusters_named <- Idents(ENs)


pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/diff_dim_pl/annotated_0_1.pdf", 
     width=8, height=5)
print(dim_pl)
...
```{r indiv-per-cluster}
ENs@meta.data$en_clusters_named <- factor(ENs@meta.data$en_clusters_named,
                                             levels = c( "Granule_1", "Ex_1_cx", "Granule_2","Ex_2_cx", "Granule_3", "Ex_3", "Ex_4","Ex_5_cx","Ex_6_cx", "Ex_7_cx","Ex_8_cx","Ex_9_hc"))

# count how many cells there are in each group and cluster
sum_donor_id_cluster <- table(ENs$en_clusters_named, ENs$donor_id)
...
```{r}
# count how many cells there are in each group and cluster
sum_donor_idtissue_cluster <- table(ENs$en_clusters_named, ENs$ProcessNumber)
# calculate the proportions, for each cluster (margin 1 for rows)
...
colSums(table(ENs$ProcessNumber, ENs$Tissue)>0)
...
# Sex
# DimPlot(ENs, split.by = "ageSex", group.by = "Tissue", ncol = 5)
DimPlot(ENs, split.by = "donor_status", group.by = "en_clusters_named", ncol = 2,
...
Idents(ENs) <- ENs$en_clusters_named
umap_clusters <- DimPlot(ENs, split.by = "Tissue", ncol = 2, 
...
```{r}
# count how many cells there are in each group and cluster
sum_tissue_cluster <- table(ENs$en_clusters_named, ENs$Tissue)
...
saveRDS(ENs, "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/20211201_ENs.rds")
