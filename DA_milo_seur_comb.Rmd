---
title: "Compositional analysis with Milo - Full data"
...
```{r}
seur_comb <- readRDS(here("06_swapped", "20211005_seur_comb_qc_integrated_annot_swappedsamples.rds")) 
...
```{r, dev="jpeg"}
plotReducedDim(seur_comb_sce, colour_by="donor_status", dimred = "UMAP", text_by = "clusters_named") 
```
...
seur_comb_milo <- buildGraph(seur_comb_milo, k = 20, d = 35, reduced.dim = "PCA")
```
...
```{r}
seur_comb_milo <- makeNhoods(seur_comb_milo, prop = 0.05, k = 20, d=35, refined = TRUE, reduced_dims = "PCA")
```
...
As a rule of thumb we want to have an average neighbourhood size over 5 x N_samples or to have a distribution peaking between 50 and 100. Otherwise you might consider rerunning makeNhoods increasing k and/or prop. 
...
umap_pl <- plotReducedDim(seur_comb_milo, dimred = "UMAP", colour_by="donor_status", text_by = "clusters_named", 
...
```{r}
da_results <- annotateNhoods(seur_comb_milo, da_results, coldata_col = "clusters_named")
...
```{r}
#save results
saveRDS(da_results, here("seur_comb_da_results.rds"))
```
...
#Oligos_milo <- buildNhoodGraph(Oligos_milo)

## Find groups
#set.seed(42)
#da_results <- groupNhoods(Oligos_milo, da_results, max.lfc.delta = 1.5, overlap = 3) #tweak parameters to optimise
#head(da_results)
#
#plotNhoodGroups(Oligos_milo, da_results, layout="UMAP") 
#plotDAbeeswarm(da_results, "NhoodGroup")
```

#Find gene signatures of neighbourhoods
```{r}
#keep.rows <- rowSums(logcounts(Oligos_milo)) != 0
#Oligos_milo <- Oligos_milo[keep.rows, ]

## Find HVGs
#dec <- modelGeneVar(Oligos_milo)
#hvgs <- getTopHVGs(dec, n=2000)
#head(hvgs)

#nhood_markers <- findNhoodGroupMarkers(Oligos_milo, da_results, subset.row = hvgs, 
#                                       aggregate.samples = TRUE, sample_col = "ProcessNumber")

#head(nhood_markers)

#gr7_markers <- nhood_markers[c("logFC_7", "adj.P.Val_7")] 
#colnames(gr7_markers) <- c("logFC", "adj.P.Val")

#head(gr7_markers[order(gr7_markers$adj.P.Val), ])

# Add Oligodendrocytes separate analysis

#For the oligodendroglia we computed the DA using the PCA reduction and a KNN graph exclusively computed for these cell types.
#We replace the oligo and opc neighboorhoods by those computed separetly.
```
