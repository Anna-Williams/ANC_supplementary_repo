...
```{r}
endoperi <- readRDS("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/20211201_endoperi.rds")
...
```{r, dev="jpeg"}
plotReducedDim(endoperi_sce, colour_by="donor_status", dimred = "UMAP", text_by = "ep_clusters_named") 
```
...
```{r}
endoperi_milo <- Milo(endoperi_sce)
endoperi_milo
```
...
endoperi_milo <- buildGraph(endoperi_milo, k = 20, d = 14, reduced.dim = "PCA")
```
...
```{r}
endoperi_milo <- makeNhoods(endoperi_milo, prop = 0.05, k = 20, d=14, refined = TRUE, reduced_dims = "PCA")
#intended for k to be 30 so try again maybe?
```
...
```{r}
plotNhoodSizeHist(endoperi_milo)
```
...
```{r}
endoperi_milo <- countCells(endoperi_milo, meta.data = as.data.frame(colData(endoperi_milo)), sample="ProcessNumber")
```
...
```{r}
head(nhoodCounts(endoperi_milo))
```
...
```{r}
design <- data.frame(colData(endoperi_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
I tried running with Tissue instad of batch as covariate, which alters the results slightly. Effects on pine and birch are most robust. 
...
```{r}
endoperi_milo <- calcNhoodDistance(endoperi_milo, d=14, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(endoperi_milo, design = ~ Tissue + donor_status, design.df = design)
head(da_results)
```
...
endoperi_milo <- buildNhoodGraph(endoperi_milo)
## Plot single-cell TSNE
umap_pl <- plotReducedDim(endoperi_milo, dimred = "UMAP", colour_by="donor_status", text_by = "ep_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(endoperi_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(endoperi_milo, da_results, coldata_col = "ep_clusters_named")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(ep_clusters_named_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$ep_clusters_named <- ifelse(da_results$ep_clusters_named_fraction < 0.7, "Mixed", da_results$ep_clusters_named)
```
...
plotDAbeeswarm(da_results, group.by = "ep_clusters_named")
```

```{r}
#save results
saveRDS(da_results, here("milo","endoperi_da_results.rds"))
```
