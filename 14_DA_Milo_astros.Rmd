---
title: "Compositional analysis with Milo - astrocyte subclusters"
...
```{r, dev="jpeg"}
plotReducedDim(astros_sce, colour_by="donor_status", dimred = "UMAP", text_by = "as_clusters_named") 
```
...
astros_milo <- buildGraph(astros_milo, k = 30, d = 12, reduced.dim = "PCA")
```
...
```{r}
astros_milo <- makeNhoods(astros_milo, prop = 0.1, k = 30, d=12, refined = TRUE, reduced_dims = "PCA")
```
...
```{r}
astros_milo <- countCells(astros_milo, meta.data = as.data.frame(colData(astros_milo)), sample="ProcessNumber")
```
...
```{r}
design <- data.frame(colData(astros_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
```{r}
astros_milo <- calcNhoodDistance(astros_milo, d=12, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(astros_milo, design = ~ Tissue + donor_status, design.df = design)
head(da_results)
```
...
astros_milo <- buildNhoodGraph(astros_milo)
## Plot single-cell TSNE
umap_pl <- plotReducedDim(astros_milo, dimred = "UMAP", colour_by="donor_status", text_by = "as_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(astros_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(astros_milo, da_results, coldata_col = "as_clusters_named")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(as_clusters_named_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$as_clusters_named <- ifelse(da_results$as_clusters_named_fraction < 0.7, "Mixed", da_results$as_clusters_named)
```
...
plotDAbeeswarm(da_results, group.by = "as_clusters_named")
```

```{r}
#save results
saveRDS(da_results, here("astros_da_results.rds"))
```
