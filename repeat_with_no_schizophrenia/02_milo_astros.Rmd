...
astro_sce <- as.SingleCellExperiment(astro, assay = "integrated")
```
...
```{r}
astro_milo <- buildGraph(astro_milo, k = 30, d = 13, reduced.dim = "PCA")
```
...
```{r}
astro_milo <- makeNhoods(astro_milo, prop = 0.1, k = 10, d=13, refined = TRUE, reduced_dims = "PCA", refinement_scheme="graph") 
```
...
```{r}
astro_milo <- countCells(astro_milo, meta.data = as.data.frame(colData(astro_milo)), sample="ProcessNumber")
```
...
```{r}
astro_design <- data.frame(colData(astro_milo))[,c("ProcessNumber", "donor_status", "Tissue")]

## Convert info from integer to factor
astro_design$Tissue <- as.factor(astro_design$Tissue) 
astro_design$donor_status <- as.factor(astro_design$donor_status)
astro_design$donor_status <- relevel(astro_design$donor_status, "Ctrl")
# simplify 
astro_design <- distinct(astro_design)
rownames(astro_design) <- astro_design$ProcessNumber
...
<!-- ```{r} -->
<!-- astro_milo <- calcNhoodDistance(astro_milo, d=13, reduced.dim = "PCA") -->
<!-- ``` -->
...
```{r}
da_results <- testNhoods(astro_milo,  design = ~ Tissue + donor_status, design.df = astro_design,  fdr.weighting="graph-overlap")
head(da_results)
```
...
astro_milo <- buildNhoodGraph(astro_milo)

## Plot single-cell UMAP
umap_pl <- plotReducedDim(astro_milo, dimred = "UMAP", colour_by="donor_status", text_by = "as_clusters_named_without_skf", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")

## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(astro_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(astro_milo, da_results, coldata_col = "as_clusters_named_without_skf")
head(da_results)
```
...
da_results$as_clusters_named_without_skf <- factor(da_results$as_clusters_named_without_skf, levels= c("Mixed","Astro_Mint",  "Astro_Basil", "Astro_Chives", 
"Astro_Sage", "Astro_Thyme", "Astro_Chervil", "Astro_Oligo"))
