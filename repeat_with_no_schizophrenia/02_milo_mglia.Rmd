...
```{r}
astro <- readRDS(here("data/repeat_with_no_schizophrenia/microglia.RDS"))
...
```{r}
mglia_milo <- buildGraph(mglia_milo, k = 30, d = 13, reduced.dim = "PCA")
```
...
```{r}
mglia_milo <- makeNhoods(mglia_milo, prop = 0.1, k = 10, d=13, refined = TRUE, reduced_dims = "PCA", refinement_scheme="graph") 
```
...
```{r}
mglia_design <- data.frame(colData(mglia_milo))[,c("ProcessNumber", "donor_status", "Tissue")]

## Convert info from integer to factor
mglia_design$Tissue <- as.factor(mglia_design$Tissue) 
mglia_design$donor_status <- as.factor(mglia_design$donor_status)
mglia_design$donor_status <- relevel(mglia_design$donor_status, "Ctrl")
# simplify 
mglia_design <- distinct(mglia_design)
rownames(mglia_design) <- mglia_design$ProcessNumber
...
<!-- ```{r} -->
<!-- mglia_milo <- calcNhoodDistance(mglia_milo, d=13, reduced.dim = "PCA") -->
<!-- ``` -->
...
```{r}
da_results <- testNhoods(mglia_milo,  design = ~ Tissue + donor_status, design.df = mglia_design,  fdr.weighting="graph-overlap")
head(da_results)
```
...
umap_pl <- plotReducedDim(mglia_milo, dimred = "UMAP", colour_by="donor_status", text_by = "as_clusters_named_without_skf", 
...
```{r}
da_results <- annotateNhoods(mglia_milo, da_results, coldata_col = "as_clusters_named_without_skf")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(as_clusters_named_without_skf_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$as_clusters_named_without_skf <- ifelse(da_results$as_clusters_named_without_skf_fraction < 0.7, "Mixed", da_results$as_clusters_named_without_skf)
```

Now we can visualize the distribution of DA Fold Changes in different cell types

```{r, fig.height=7, fig.width=7, dev="jpeg"}
da_results$as_clusters_named_without_skf <- factor(da_results$as_clusters_named_without_skf, levels= c("Mixed","Mglia_Rose", "Mglia_Violet", "Mglia_Lily", "Mglia_Daisy", 
"Mglia_Tulip",  "Mglia_BAMs","Mglia_Lupin", "Mglia_Pansy"))
plotDAbeeswarm(da_results, group.by = "as_clusters_named_without_skf")
ggsave(here("outs/repeat_with_no_schizophrenia/abundance/milo_astro.pdf"))
```
