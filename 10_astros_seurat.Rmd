...
The purpose of this script is to find variable genes, cluster the astrocyte
...
astros <-subset(seur_comb, idents = c("Astro1", "Astro2"))
...
Idents(astros) <- "Tissue"
VlnPlot(astros, features = c("nFeature_RNA", 
                             "nCount_RNA", 
                             "percent.mt"))
...
```{r}
plot1 <- FeatureScatter(astros, 
                        feature1 = "nCount_RNA", 
                        feature2 = "percent.mt", 
                        cols = mycoloursP)

plot2 <- FeatureScatter(astros, 
                        feature1 = "nCount_RNA", 
...
DefaultAssay(astros) <-"integrated"
...
#top10 <- head(VariableFeatures(astros), 10)

# plot variable features with and without labels
#plot1 <- VariableFeaturePlot(astros)
...
```{r}
astros <- RunPCA(astros, features = VariableFeatures(object = astros))
ElbowPlot(astros)

```

```{r}
#astros@graphs$integrated_snn <- "integrated_snn"

astros <- FindNeighbors(astros, dims = 1:12)

# test different resolutions for clustering
astros <- FindClusters(astros, resolution = c(0.01, 0.04, 0.06, 0.08,
                                              seq(from = 0.1, to = 1, by = 0.1)), graph.name = "integrated_snn")

astros@meta.data$integrated_snn_res.0.25 <- NULL

# non-linear reduction
astros <- RunUMAP(astros, dims = 1:12)
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out")

dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/diff_dim_pl")
plot_list_func(seur_obj = astros, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/diff_dim_pl/" )
...
DimPlot(astros, group.by = "Tissue", split.by = "Tissue")
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/clust_tree")

pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/clust_tree/clust_tree.pdf", 
...
```{r}
astros_sce <- as.SingleCellExperiment(astros, assay = "integrated")
```
...
sil_plot <- function(sce_obj, reduction = "PCA",
  ...
  met_dat <- as.data.frame(colData(astros_sce))
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/sil_pl")

av_df <- sil_plot(sce_obj = astros_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/sil_pl/" )
...
pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/sil_pl/av_sil_pl.pdf", 
     width=8, height=4)
print(av_sil_pl)
dev.off()
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/appr_sil")


approx_sil(sce_obj = astros_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/appr_sil/" )
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/clust_pure")


clu_pure(sce_obj = astros_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/clust_pure/")
...
```{r}
Idents(astros) <- "integrated_snn_res.0.2"
  astros <-  RenameIdents(
    astros,
    "0" = "Astro_1",
    "1" = "Astro_2",
    ...
    "6" = "Astro_6_CB")
# Plot result
DimPlot(astros,label = TRUE, repel=TRUE)
dim_pl <- DimPlot(astros,label = TRUE, repel=FALSE, cols = mycoloursP[6:40],
                  label.size = 4.5)
dim_pl
# Save result
...
pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/diff_dim_pl/annotated_0_1.pdf", 
     width=8, height=5)
print(dim_pl)
...
astros@meta.data$as_clusters_named <- factor(astros@meta.data$as_clusters_named,
                                             levels = c("Astro_1", "Astro_2", "Astro_3", "Astro_4_CB", "Astro_5", "Astro_6_CB", "Astro_Oligo"))

# count how many cells there are in each group and cluster
sum_donor_id_cluster <- table(astros$as_clusters_named, astros$donor_id)
...
```{r}
# count how many cells there are in each group and cluster
sum_donor_idtissue_cluster <- table(astros$as_clusters_named, astros$ProcessNumber)
# calculate the proportions, for each cluster (margin 1 for rows)
...
# Compositional differences with condition and region
...
colSums(table(astros$ProcessNumber, astros$Tissue)>0)
...
DimPlot(astros, split.by = "donor_status", group.by = "as_clusters_named", ncol = 2,
...
Idents(astros) <- astros$as_clusters_named
umap_clusters <- DimPlot(astros, split.by = "Tissue", ncol = 2, 
...
```{r}
# count how many cells there are in each group and cluster
sum_tissue_cluster <- table(astros$as_clusters_named, astros$Tissue)
...
saveRDS(astros, "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/astros_out/20211203_astros.rds")
