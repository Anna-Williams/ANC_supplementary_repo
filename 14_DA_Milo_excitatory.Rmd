---
title: "Compositional analysis with Milo - not controlled for tissue for ENs"
...
```{r}
ENs <- readRDS("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/ENs_out/20211201_ENs.rds")
...
```{r, dev="jpeg"}
plotReducedDim(ENs_sce, colour_by="donor_status", dimred = "UMAP", text_by = "en_clusters_named") 
```
...
ENs_milo <- buildGraph(ENs_milo, k = 35, d = 9, reduced.dim = "PCA")
```
...
- `refined`: indicates whether you want to use the sampling refinement algorithm, or just pick cells at random. The default and recommended way to go is to use refinement. The only situation in which you might consider using `random` ENstead, is if you have batch corrected your data with a graph based correction algorithm, such as [BBKNN](https://github.com/Teichlab/bbknn), but the results of DA testing will be suboptimal.

```{r}
ENs_milo <- makeNhoods(ENs_milo, prop = 0.05, k = 35, d=9, refined = TRUE, reduced_dims = "PCA")
```
...
```{r}
plotNhoodSizeHist(ENs_milo)
```

## Counting cells in neighbourhoods

_Milo_ leverages the variation in cell numbers between replicates for the same experimental condition to test for differential abundance. Therefore we have to count how many cells from each sample are in each neighbourhood. We need to use the cell metadata and specify which column contaENs the sample information.

```{r}
ENs_milo <- countCells(ENs_milo, meta.data = as.data.frame(colData(ENs_milo)), sample="ProcessNumber")
```
...
```{r}
design <- data.frame(colData(ENs_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
I tried running with Tissue ENstad of batch as covariate, which alters the results slightly. Effects on pine and birch are most robust. 
...
```{r}
ENs_milo <- calcNhoodDistance(ENs_milo, d=9, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(ENs_milo, design =  ~ donor_status, design.df = design)
head(da_results)
```
...
# ENspecting DA testing results

We can start ENspecting the results of our DA analysis from a couple of standard diagnostic plots. 
We first ENspect the distribution of uncorrected P values, to verify that the test was balanced.
...
ENs_milo <- buildNhoodGraph(ENs_milo)
## Plot single-cell TSNE
umap_pl <- plotReducedDim(ENs_milo, dimred = "UMAP", colour_by="donor_status", text_by = "en_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(ENs_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(ENs_milo, da_results, coldata_col = "en_clusters_named")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(en_clusters_named_fraction)) + geom_histogram(bENs=50)
```
```{r}
da_results$en_clusters_named <- ifelse(da_results$en_clusters_named_fraction < 0.7, "Mixed", da_results$en_clusters_named)
```
...
plotDAbeeswarm(da_results, group.by = "en_clusters_named")
```

```{r}
#save results
saveRDS(da_results, here("milo","ENs_da_results.rds"))
```
