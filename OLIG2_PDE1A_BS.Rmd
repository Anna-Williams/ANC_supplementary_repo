---
title: "Validation of OLIG2/PDE1A with BaseScope"
...
```{r}
#IHC HC PDE1A_Olig2
dat_BS <- read.csv("M:/PhD/Year 4/Validation/BaseScope/OLIG2_PDE1A_BS.csv")
...
#unblind, add disease status 
dat_BS$Condition <- ifelse(dat_BS$ID == "1" | dat_BS$ID == "2" | dat_BS$ID == "4" | dat_BS$ID == "7" |dat_BS$ID == "8" | dat_BS$ID == "3",  "HD", "Ctrl")

#calculate double positives as proportion of total OLIG2
dat_BS$prop_dPos <- dat_BS$dPos/dat_BS$OLIG2
```
...
#Mann-Whitey U test for condition
MU_cond <- wilcox.test(prop_dPos ~ Condition, data = dat_BS)
MU_cond#W = 600, p-value = 0.2024

#adjust for multiple comparison
MU_cond$p.adj <- p.adjust(MU_cond$p.value, "hochberg", n = 2)
MU_cond$p.adjc #0.4047206

#Mann-Whitey U test for tissue
MU_tis <- wilcox.test(prop_dPos ~ tissue, data = dat_BS)
MU_tis #W = 738, p-value = 0.001662

#adjust for multiple comparison 
MU_tis$p.adj <- p.adjust(MU_tis$p.value, "hochberg", n = 2)
MU_tis$p.adj #0.003324457
...
p2 <- ggplot(dat_BS, aes(x = Condition, y = prop_dPos, fill = Condition)) +
  ...
  theme_classic() + ylab("Proportion of OLIG2+ cells also +ive for PDE1A") + xlab("Condition") + 
  scale_fill_manual(values = (brewer.pal(10,"Paired")[3:4])) + theme(axis.text = element_text(size = 20))  +
  theme(axis.title = element_text(size = 25))  + theme(legend.text = element_text(size = 20))  + theme(legend.title = element_text(size = 20)) + ylim(0,.8)



p3 <- ggplot(dat_BS, aes(x = Myelinated, y = prop_dPos, fill = Myelinated)) +
  geom_boxplot()+ geom_jitter(position=position_dodge(width=0.75)) +
  theme_classic() + ylab("Proportion of OLIG2+ cells also +ive for PDE1A") + xlab("Myelinated") + 
...
pdf(here("PhD","Thesis","thesisplots","20221017_OLIG2_PDE1A_BS_boxplots.pdf"), width = 7, height = 9)
print(PlotList)
dev.off()
