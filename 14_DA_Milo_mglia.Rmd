...
```{r, dev="jpeg"}
plotReducedDim(mglia_sce, colour_by="donor_status", dimred = "UMAP", text_by = "mm_clusters_named") 
```
...
mglia_milo <- buildGraph(mglia_milo, k = 30, d = 14, reduced.dim = "PCA")
```
...
```{r}
mglia_milo <- makeNhoods(mglia_milo, prop = 0.1, k = 30, d=14, refined = TRUE, reduced_dims = "PCA")
...
```{r}
mglia_milo <- countCells(mglia_milo, meta.data = as.data.frame(colData(mglia_milo)), sample="ProcessNumber")
```
...
```{r}
design <- data.frame(colData(mglia_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
```{r}
mglia_milo <- calcNhoodDistance(mglia_milo, d=14, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(mglia_milo, design = ~ Tissue + donor_status, design.df = design)
head(da_results)
```
...
mglia_milo <- buildNhoodGraph(mglia_milo)
## Plot single-cell TSNE
umap_pl <- plotReducedDim(mglia_milo, dimred = "UMAP", colour_by="donor_status", text_by = "mm_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(mglia_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(mglia_milo, da_results, coldata_col = "mm_clusters_named")
#da_results <- annotateNhoods(mglia_milo, da_results, coldata_col = "celltype")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(mm_clusters_named_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$mm_clusters_named <- ifelse(da_results$mm_clusters_named_fraction < 0.7, "Mixed", da_results$mm_clusters_named)
```
...
plotDAbeeswarm(da_results, group.by = "mm_clusters_named")
```

```{r}
#save results
saveRDS(da_results, here("mglia_da_results.rds"))
```
