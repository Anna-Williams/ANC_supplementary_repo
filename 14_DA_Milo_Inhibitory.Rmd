---
title: "Compositional analysis with Milo - try not controlling for tissue for neurons"
...
```{r}
INs <- readRDS("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/INs_out/20220105_INs.rds")
...
```{r, dev="jpeg"}
plotReducedDim(INs_sce, colour_by="donor_status", dimred = "UMAP", text_by = "in_clusters_named") 
```
...
INs_milo <- buildGraph(INs_milo, k = 20, d = 9, reduced.dim = "PCA")
```
...
```{r}
INs_milo <- makeNhoods(INs_milo, prop = 0.1, k = 20, d=9, refined = TRUE, reduced_dims = "PCA")
...
```{r}
INs_milo <- countCells(INs_milo, meta.data = as.data.frame(colData(INs_milo)), sample="ProcessNumber")
```
...
```{r}
design <- data.frame(colData(INs_milo))[,c("ProcessNumber", "donor_status", "Tissue")]
...
```{r}
INs_milo <- calcNhoodDistance(INs_milo, d=9, reduced.dim = "PCA")
```
...
```{r}
da_results <- testNhoods(INs_milo, design =  ~ donor_status, design.df = design)
head(da_results)
```
...
INs_milo <- buildNhoodGraph(INs_milo)
## Plot single-cell TSNE
umap_pl <- plotReducedDim(INs_milo, dimred = "UMAP", colour_by="donor_status", text_by = "in_clusters_named", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(INs_milo, da_results, layout="UMAP",alpha=0.1) 
...
```{r}
da_results <- annotateNhoods(INs_milo, da_results, coldata_col = "in_clusters_named")
head(da_results)
```
...
```{r}
ggplot(da_results, aes(in_clusters_named_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$in_clusters_named <- ifelse(da_results$in_clusters_named_fraction < 0.7, "Mixed", da_results$in_clusters_named)
```
...
plotDAbeeswarm(da_results, group.by = "in_clusters_named")
```

```{r}
#save results
saveRDS(da_results, here("milo","INs_da_results.rds"))
```
