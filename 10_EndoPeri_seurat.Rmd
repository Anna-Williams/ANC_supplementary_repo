...
The purpose of this script is to find variable genes, cluster the EC/Pericyte
...
endoperi <-subset(seur_comb, idents = "ECs/Peri")
...
Idents(endoperi) <- "Tissue"
VlnPlot(endoperi, features = c("nFeature_RNA", 
                             "nCount_RNA", 
                             "percent.mt"))
...
```{r}
plot1 <- FeatureScatter(endoperi, 
                        feature1 = "nCount_RNA", 
                        feature2 = "percent.mt", 
                        cols = mycoloursP)

plot2 <- FeatureScatter(endoperi, 
                        feature1 = "nCount_RNA", 
...
DefaultAssay(endoperi) <-"integrated"

endoperi <- FindVariableFeatures(endoperi, 
                               selection.method = "vst", 
                               nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(endoperi), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(endoperi)
#lot2 <- LabelPoints(plot = plot1, 
#                     points = top10, 
#                     repel = TRUE)
plot1 + plot2
...
```{r}
endoperi <- RunPCA(endoperi, features = VariableFeatures(object = endoperi))
ElbowPlot(endoperi)

```

```{r}
#endoperi@graphs$integrated_snn <- "integrated_snn"

endoperi <- FindNeighbors(endoperi, dims = 1:14)

# test different resolutions for clustering
endoperi <- FindClusters(endoperi, resolution = c(0.04, 0.06, 0.08,
                                              seq(from = 0.1, to = 1, by = 0.1)), graph.name = "integrated_snn")

endoperi@meta.data$integrated_snn_res.0.25 <- NULL

# non-linear reduction
endoperi <- RunUMAP(endoperi, dims = 1:14)
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out")

dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/diff_dim_pl")
plot_list_func(seur_obj = endoperi, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/diff_dim_pl/" )
...
DimPlot(endoperi, group.by = "Tissue", split.by = "Tissue")
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/clust_tree")

pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/clust_tree/clust_tree.pdf", 
...
```{r}
endoperi_sce <- as.SingleCellExperiment(endoperi, assay = "integrated")
#endoperi_sce <- readRDS("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/20211027_endoperi_SCE.rds")
```
...
sil_plot <- function(sce_obj, reduction = "PCA",
  ...
  met_dat <- as.data.frame(colData(endoperi_sce))
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/sil_pl")

av_df <- sil_plot(sce_obj = endoperi_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/sil_pl/" )
...
pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/sil_pl/av_sil_pl.pdf", 
     width=8, height=4)
print(av_sil_pl)
dev.off()
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/appr_sil")


approx_sil(sce_obj = endoperi_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/appr_sil/" )
...
dir.create("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/clust_pure")


clu_pure(sce_obj = endoperi_sce, 
                          col_pattern = "integrated_snn_res.",
                          plot_cols = mycoloursP[6:40],
                          save_dir = "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/clust_pure/")
...
```{r}
Idents(endoperi) <- "integrated_snn_res.0.2"
  endoperi <-  RenameIdents(
    endoperi,
    "0" = "EC_cap_1",
    "1" = "Peri_cap_1",
    "2" = "Peri_cap_2",
    ...
    "7" = "EC_art_2")
# Plot result
DimPlot(endoperi,label = TRUE, repel=TRUE)
dim_pl <- DimPlot(endoperi,label = TRUE, repel=FALSE, cols = mycoloursP[6:40],
                  label.size = 4.5)
dim_pl
# Save result
endoperi$ep_clusters_named <- Idents(endoperi)


pdf("/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/diff_dim_pl/annotated_0_02.pdf", 
     width=8, height=5)
print(dim_pl)
...
```{r indiv-per-cluster}
endoperi@meta.data$ep_clusters_named <- factor(endoperi@meta.data$ep_clusters_named,
                                             levels = c("Peri_cap_1", "Peri_cap_2", "Peri_vein", "EC_cap_1", "EC_cap_2", "EC_art_1", "EC_art_2", "vSMC"))

# count how many cells there are in each group and cluster
sum_donor_id_cluster <- table(endoperi$ep_clusters_named, endoperi$donor_id)
...
```{r}
# count how many cells there are in each group and cluster
sum_donor_idtissue_cluster <- table(endoperi$ep_clusters_named, endoperi$ProcessNumber)
# calculate the proportions, for each cluster (margin 1 for rows)
...
colSums(table(endoperi$ProcessNumber, endoperi$Tissue)>0)


#number of nuclei per tissue
table(endoperi$Tissue)
...
# Sex
# DimPlot(endoperi, split.by = "ageSex", group.by = "Tissue", ncol = 5)
DimPlot(endoperi, split.by = "donor_status", group.by = "ep_clusters_named", ncol = 2,
...
Idents(endoperi) <- endoperi$ep_clusters_named
umap_clusters <- DimPlot(endoperi, split.by = "Tissue", ncol = 2, 
...
```{r}
# count how many cells there are in each group and cluster
sum_tissue_cluster <- table(endoperi$ep_clusters_named, endoperi$Tissue)
...
saveRDS(endoperi, "/exports/eddie/scratch/s1359339/HD/20211005_swapped_data/endoperi_out/20211201_endoperi.rds")
